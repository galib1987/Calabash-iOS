...
{
    [self pushViewWithMessage:message shouldAutoDismiss:NO];
}
     ...
     dispatch_get_main_queue(), ^
     {
         @synchronized(self)
         {
                 ...
                 (self.messageStack.count < 4 && [self.messageStack objectForKey:message.text] == nil))
             {
                 ...
                 msgView.backgroundColor = message.backgroundColor ?: self.backgroundColor;
                 msgView.font = self.font;
                 msgView.messageText = message.text;
                 msgView.shouldAutoDismiss = shouldAutoDismiss || message.shouldAutoDismiss;
                          ...
                          [self performBlock:^{
                              [UIView animateWithDuration:.35
                                               animations:^
                               {
                                   ...
                                   [self popViewWithText:msgView.messageText];
                                   [msgView removeFromSuperview];
                               }];

                          } afterDelay:self.autoDismissDelay];
                      }
