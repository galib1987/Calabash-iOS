...
+ (NSString*)connectedWifiName
{
    ...
    for (NSString *ifnam in ifs)
    {
        ...
        if (info[@"SSID"])
        {
            ssid = [NSString stringWithFormat:@"%@", info[@"SSID"]];
        }
    }
...
+ (NSNumber*)freeMemory
{
    ...
    return [NSNumber numberFromObject:[NSString stringWithFormat:@"%10.0f", vm_stat.free_count * pagesize / 1024.0f]];
}
...
+ (NSNumber*)freeDiskspace
{
    ...
    if (dictionary)
    {
        freeFileSystemSizeInBytes = [NSNumber numberWithLongLong:[[dictionary objectForKey:NSFileSystemFreeSize] unsignedLongLongValue] / 1024.0f];
    }
    else
    {
