...
// Make changes to NJOPAirport.m instead.

#import "_NJOPAirport.h"

const struct NJOPAirportAttributes NJOPAirportAttributes = {
...
@implementation NJOPAirportID
@end

@implementation _NJOPAirport
...
- (NJOPAirportID*)objectID {
	return (NJOPAirportID*)[super objectID];
}
...
- (void)setCustoms_availableValue:(BOOL)value_ {
	[self setCustoms_available:@(value_)];
}
...
- (void)setPrimitiveCustoms_availableValue:(BOOL)value_ {
	[self setPrimitiveCustoms_available:@(value_)];
}
...
- (void)setPrimitiveElevation_qtyValue:(int16_t)value_ {
	[self setPrimitiveElevation_qty:@(value_)];
}
...
- (void)setPrimitiveFuel_availableValue:(BOOL)value_ {
	[self setPrimitiveFuel_available:@(value_)];
}
...
- (void)setInstrument_approach_flagValue:(BOOL)value_ {
	[self setInstrument_approach_flag:@(value_)];
}
...
- (void)setPrimitiveInstrument_approach_flagValue:(BOOL)value_ {
	[self setPrimitiveInstrument_approach_flag:@(value_)];
}
...
- (void)setLongest_runway_length_qtyValue:(int16_t)value_ {
	[self setLongest_runway_length_qty:@(value_)];
}
...
- (void)setPrimitiveLongest_runway_length_qtyValue:(int16_t)value_ {
	[self setPrimitiveLongest_runway_length_qty:@(value_)];
}
...
- (void)setPrimitiveSlots_requiredValue:(BOOL)value_ {
	[self setPrimitiveSlots_required:@(value_)];
}
