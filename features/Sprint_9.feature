Feature: Welcome Screen, Share Flight Details and Add to Calendar
  ...
  Scenario: iPhone - User Shares Flight Details via email

    Given: User Logged in with a valid userId and password
    When : User Try to Share Flight Information with others by email with a note
    Then : An Email Should be send with flight details information to the destination email address


  Scenario: iPhone - User Shares Flight Details via SMS

    Given: User Logged in with a valid userId and password
    When : User Try to Share Flight Information with others by SMS with a note
    Then : A SMS Should be send with flight details information to the destination Phone Number


  Scenario: iPhone - User try to add flight information to the calendar

    Given: User Logged in with a valid userId and password
    When : User Try to Add Flight Information to the calender
    Then : User should be taken to the calender with all the pre populated fields
    And  : User Should be successfully add flight information to the calendar.
